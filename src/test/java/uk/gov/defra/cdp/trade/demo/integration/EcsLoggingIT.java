...
class EcsLoggingIT extends IntegrationBase {
  ...
  void healthEndpoint_shouldBeFilteredFromLogs() throws Exception {
    // Temporarily disable WebTestClient DEBUG logging to prevent client-side /health logs
    Logger exchangeFunctionsLogger =
        (Logger)
            LoggerFactory.getLogger(
                "org.springframework.web.reactive.function.client.ExchangeFunctions");
    Level originalLevel = exchangeFunctionsLogger.getLevel();
    exchangeFunctionsLogger.setLevel(Level.INFO);
    ...
    try {
      ...
      exchangeFunctionsLogger.setLevel(originalLevel);
    }
  }
  ...
  void missingTraceIdHeader_shouldLeaveTraceIdEmpty() throws Exception {
    ...
    // Create a webClient WITHOUT the default x-cdp-request-id header
    WebTestClient clientWithoutTraceHeader =
        WebTestClient.bindToServer()
            .baseUrl("http://localhost:%d".formatted(port))
            .defaultHeader("Content-Type", "application/json")
            // Note: No x-cdp-request-id header here
            .build();

    clientWithoutTraceHeader.get().uri("/actuator/info").exchange().expectStatus().isNotFound();
